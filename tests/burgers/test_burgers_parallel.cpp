...
constexpr unsigned int net_size = 50;
using MyPINN = PINN<
...
struct Point { Real x, t; Real target; };
struct Data { 
...
static Real get_u(const Real* xt, const Real* p) {
    return MyPINN::forward_static(xt, p);
}
...
static Real get_ux(const Real* xt, const Real* p) {
    return calc_du(xt, p, 0); 
}
...
static Real weighted_loss_bc(const Real* p, const Real* xt, Real weight) {
    return weight * loss_bc_single(p, xt);
}

static Real weighted_loss_pde(const Real* p, const Real* xt, Real weight) {
    return weight * loss_pde_single(p, xt);
}
...
int main() {
        ...
        #pragma omp parallel
        {
            ...
            for(size_t i=0; i<data.initial.size(); ++i) {
                ...
                __enzyme_autodiff((void*)weighted_loss_ic, 
                                  enzyme_dup, p_ptr, local_grad.data(), 
                                  enzyme_const, xt, 
                                  enzyme_const, data.initial[i].target, 
                                  enzyme_const, s_ic);
            }
            ...
            for(size_t i=0; i<data.boundary.size(); ++i) {
                Real xt[2] = {data.boundary[i].x, data.boundary[i].t};
                __enzyme_autodiff((void*)weighted_loss_bc, 
                                  enzyme_dup, p_ptr, local_grad.data(), 
                                  enzyme_const, xt, 
                                  enzyme_const, s_bc);
            }
            ...
            for(size_t i=0; i<data.collocation.size(); ++i) {
                Real xt[2] = {data.collocation[i].x, data.collocation[i].t};
                __enzyme_autodiff((void*)weighted_loss_pde, 
                                  enzyme_dup, p_ptr, local_grad.data(), 
                                  enzyme_const, xt, 
                                  enzyme_const, s_pde);
            }
    ...
    write_grid(0.0, 0); write_grid(0.5, 0); write_grid(1.0, 0); write_grid(1.5, 2);
    outFile.close();
