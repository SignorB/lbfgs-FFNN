...
V SLBFGS<V,M>::stochastic_solve(std::vector<V> inputs, std::vector<V> targets, V weights,const S_VecFun &f,const S_GradFun &S_GradFun, int m, int M_param, int L, int b, int b_H, double step_size, int N, bool verbose, int print_every) {
    ...
    for (int t=0; t < m ; ++t){
      ...
      #pragma omp parallel
      {
          V thread_grad_wt = V::Zero(dim_weights);
          V thread_grad_wk = V::Zero(dim_weights);
          ...
          for (size_t i=0; i < minibatch_indices.size(); ++i) {
            ...
            thread_grad_wt += temp_grad_estimate_wt;
            S_GradFun(weights, inputs[idx], targets[idx], temp_grad_estimate_wk);
            thread_grad_wk += temp_grad_estimate_wk;
          }
          
          #pragma omp critical
          {
              grad_estimate_wt += thread_grad_wt;
              grad_estimate_wk += thread_grad_wk;
          }
      }
      ...
      */
      ...
      if (t%L==0 && t>0){
        ...
        if (!u_list.empty()) {
          ...
          #pragma omp parallel
          {
              V local_y = V::Zero(dim_weights);
              #pragma omp for
              for (size_t i = 0; i < minibatch_indices_H.size(); ++i) {
                const size_t idx = minibatch_indices_H[i];
                local_y += finite_difference_hvp(S_GradFun, u, inputs[idx], targets[idx], s);
              }
              #pragma omp critical
              y += local_y;
          }
